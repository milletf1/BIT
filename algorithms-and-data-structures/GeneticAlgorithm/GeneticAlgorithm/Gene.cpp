...
array<Chromosome^>^ Gene::reproduce(Chromosome^ a, Chromosome^ b)
{
	...
	int splitPosition = myRandom->Next(1, 9);	// determine the value split position.
...
{ 
	...
	for(int i = 0; i <= 1; i++)
	{
		...
		// check if the first chromosome in the myChromosome array was chosen and it hasn't already been picked.		
		if(roll <= myChromosomes[0]->getFitness() && 
			(i == 0 || otherParent != 0))	
		{			
			pair[i] = myChromosomes[0];
			otherParent = 0;
		...
		{		
			...
			int subTotal = myChromosomes[0]->getFitness() + myChromosomes[1]->getFitness();
			...
			{	
				// check that the chromosome we are checking for success is eligible to be selected.
				if( i == 0 || i != parentIndex)
				{
				...
				if(!selectedParent)
				{
					...
					parentIndex++;
					// check if we need to reroll ( will happen when we chose an already selected parent).
					if(parentIndex >= breedingChromosomes)
					{
						// reset subtotal and parent index. reroll selection number.
						subTotal = myChromosomes[0]->getFitness();
						parentIndex = 0;
						roll = myRandom->Next(total);
					}
					else
						subTotal += myChromosomes[parentIndex]->getFitness();
				}
			}			
			pair[i] = myChromosomes[parentIndex];
		}
	}
	return pair; 
}
...
void Gene::mutate(Chromosome^ %c)
{
	...
	int mutatePoint = myRandom->Next(10);

	// chose a random integer to mutate into.
	String^ mutateValue = Convert::ToString(myRandom->Next(10));
