...
TBlock::TBlock(Point^ startDrawPoint, Point^startOffset, Color startColor, Graphics^ startCanvas)
...
void TBlock::Rotate()
{
	...
	if(myBlockDirection == BlockDirection::North)
	{
		...
		//Set cell 3
		newPoint = gcnew Point(blockArray[3]->getGridPoint()->X - 1, blockArray[3]->getGridPoint()->Y - 1);
...
void TBlock::InitializeBlockArray(Point^ startDrawPoint)
{
	//Declare a temporary Point for passing the starting grid point to each cell
	Point^ newPoint;
