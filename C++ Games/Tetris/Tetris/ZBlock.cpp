...
ZBlock::ZBlock(Point^ startDrawPoint, Point^startOffset, Color startColor, Graphics^ startCanvas)
...
void ZBlock::Rotate()
{
	...
	if(myBlockDirection == BlockDirection::North)
	{
		...
		//Set cell 3
		newPoint = gcnew Point(blockArray[3]->getGridPoint()->X - 2, blockArray[3]->getGridPoint()->Y);
...
void ZBlock::InitializeBlockArray(Point^ startDrawPoint)
{
	//Declare a temporary Point for passing the starting grid point to each cell
	Point^ newPoint;
	...
	//set cell 3
	newPoint = gcnew Point(startDrawPoint->X + 3, startDrawPoint->Y + 1);
	blockArray[3]->setGridPoint(newPoint);
}
